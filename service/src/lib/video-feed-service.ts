    ...
    WITH candidate_items AS (
      SELECT
        ...
        CASE
          ...
          WHEN t.source IN ('heiliao', 'cg91', 'baoliao51', 'douyin', '18mh', 'rou', 'dadaafa', '18j', 'tikporn', '91porna', '91porn') THEN t.source
          WHEN t.kind = 'keyword' THEN 'search:' || t.value
          ELSE t.value
        END AS target,
        CASE
          WHEN t.source IN ('heiliao', 'cg91', 'baoliao51', 'douyin', '18mh', 'rou', 'dadaafa', '18j', 'tikporn', '91porna', '91porn') THEN t.value
      ...
      WHERE i.video_url IS NOT NULL
        ...
        AND (
          t.source NOT IN ('youtube', 'heiliao', 'cg91', 'baoliao51', 'douyin', '18mh', 'rou', 'dadaafa', '18j', 'tikporn', '91porna', '91porn')
...
export async function recordVideoEvent(input: {
  ...
  await sql`
    ...
    WHERE i.id = ${input.itemId}
      ...
      AND i.expires_at > NOW()
      AND (
        NOT EXISTS (SELECT 1 FROM targets t WHERE t.id = i.target_id AND t.source IN ('youtube', 'heiliao', 'cg91', 'baoliao51', 'douyin', '18mh', 'rou', 'dadaafa', '18j', 'tikporn', '91porna', '91porn'))
